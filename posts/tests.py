...
class PostDetailViewTest(APITestCase):
    def setUp(self):
        ...
        Post.objects.create(
            owner=brian, title='a title 2', content='brian`s content'
        )
    ...
    def test_can_retrive_post_with_invalid_id(self):
        response = self.client.get('/posts/99999/')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        
    def test_user_can_update_own_post_(self):
        self.client.login(username='adam', password='pass')
    ...
    def test_user_cant_update_not_own_post_(self):
